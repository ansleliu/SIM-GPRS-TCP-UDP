...
ansleTCPServerMain::ansleTCPServerMain(void)
{
	//创建线程退出事件句柄
...
【入口参数】:  lparam:无类型指针，可以通过此参数，向线程中传入需要用到的资源。
			   在这里我们将ansleTCPServerMain类实例指针传进来
【出口参数】:  (无)
【返回  值】:  返回值没有特别的意义，在此我们将返回值设为0。
------------------------------------------------------------------*/
DWORD ansleTCPServerMain::SocketThreadFunc(PVOID lparam)
{
	ansleTCPServerMain *pSocket;
	//得到ansleTCPServerMain实例指针
	pSocket = (ansleTCPServerMain*)lparam;
	//定义读事件集合
	fd_set fdRead;
	int ret;
	TIMEVAL	aTime;
	...
	while (TRUE)
	{
		...
		if (ret > 0)
		{
			...
			if (FD_ISSET(pSocket->m_ServerSocket,&fdRead))
			{
				...
				ansleTCPCustom * pClientSocket = new ansleTCPCustom();
...
//删除客户端
void ansleTCPServerMain::RemoteClient(ansleTCPCustom *pClient /*客户端对象*/)
{
	POSITION posPrior;
	...
	{
		posPrior = pos;
        ansleTCPCustom *pTcpCustom = (ansleTCPCustom*)m_ListClientSocket.GetNext(pos);
...
/*-----------------------------------------------------------------
【函数介绍】:  发送数据
【入口参数】:  pCustom ：客户端对象指针
			   buf : 缓冲区
			   dwBufLen : 缓冲区长度
【出口参数】:  (无)
【返回  值】:  TRUE : 发送成功 ； FALSE : 发送失败
------------------------------------------------------------------*/
BOOL ansleTCPServerMain::SendData(ansleTCPCustom* pCustom, const char * buf , DWORD dwBufLen)
{
	BOOL bResult = FALSE;
	BOOL bExisted = FALSE;
	if (pCustom == NULL)
	...
	bResult =  pCustom->SendData(buf,dwBufLen);

	if (!bResult)
	{
		//
		RemoteClient(pCustom);
	}
