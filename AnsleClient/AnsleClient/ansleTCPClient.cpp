...
在这里我们将ansleTCPClient类实例指针传进来
【出口参数】:  (无)
【返回  值】:  返回值没有特别的意义，在此我们将返回值设为0。
------------------------------------------------------------------*/
DWORD ansleTCPClient::SocketThreadFunc(LPVOID lparam)
{
	ansleTCPClient *pSocket;
	//得到ansleTCPClient实例指针
	pSocket = (ansleTCPClient*)lparam;
	//定义读事件集合
	fd_set fdRead;
	int ret;
	//定义事件等待时间
...
-----------------------------------------------------------------*/
BOOL ansleTCPClient::Open(void * pOwner)
{
	//复位线程退出事件
...
------------------------------------------------------------------*/
BOOL ansleTCPClient::SendData(const char * buf , DWORD dwBufLen)
{
	int nBytes = 0;
	UINT nSendBytes=0;
